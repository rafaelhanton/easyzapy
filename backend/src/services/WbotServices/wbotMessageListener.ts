...
const verifyQueue = async (
  ...
  if (queues.length === 1) {
    ...
    if (
      ...
      logger.info(`Integração encontrada: ${JSON.stringify(integrations)}`)
...
export const handleMessageIntegration = async (
  ...
  if (queueIntegration.type === "n8n" || queueIntegration.type === "webhook") {
    ...
    logger.error("entrou no n8n")

    logger.error("URL N8N", queueIntegration?.urlN8N)
    ...
    console.log("entrou no else, sem integração")
